  ...
  return new Promise((resolve, reject) => {
    ...
    if (!fs__namespace.existsSync(backendExecutablePath)) {
      ...
      if (electron.app.isPackaged) {
        electron.dialog.showErrorBox("Error Crítico de Backend", `El archivo del backend no se encontró en la ruta esperada: ${backendExecutablePath}. La aplicación no puede continuar.`);
      }
    ...
    backendProcess.on("error", (err) => {
      ...
      if (electron.app.isPackaged) {
        electron.dialog.showErrorBox("Error de Backend", "No se pudo iniciar el proceso del backend: " + err.message);
      }
      backendProcess = null;
      reject(err);
    });
    backendProcess.on("exit", (code, signal) => {
      ...
      if (code !== 0 && electron.app.isPackaged && !electron.app.isQuitting) {
        electron.dialog.showErrorBox("Error de Backend", `El proceso del backend terminó inesperadamente. Código: ${code}, Señal: ${signal}. Por favor, reinicie la aplicación.`);
      }
      backendProcess = null;
    });
    let retries = 0;
